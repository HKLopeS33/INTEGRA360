...
export async function getCompanyById(companyId: string): Promise<CompanyRow | null> {
  const { data, error } = await supabase
    ...
    .select('id,name,email,cnpj,phone,address,city,state,country,pixKey,kitchenPrinter,cashierPrinter,printingDisabled,active,subscription:Subscription(id,status,monthlyFee,expiresAt,lastRenewed,lastLoginAt)')
    .eq('id', companyId)
    .maybeSingle();
